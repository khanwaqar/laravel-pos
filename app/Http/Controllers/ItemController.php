...
class ItemController extends Controller
{
    ...
    {
        if ($request->ajax()) {
            $data['stock_limit_items'] = (new Item())->getItemInStock();
           
            return $this->sendCommonResponse($data, null, 'stock');
        }
    ...
    {
        $data['item'] = Item::with('attributes')->where('id',$id)->first();
    ...
    {
        $input = $request->all();
        if (!empty($input['action']) && $input['action'] == 'update-type') {
            $input = Arr::except($input,['action']);
            $item = (new Item())->saveData($input, $id);
        } else{
            $this->validator($input)->validate();
            $item = Item::find($id);
            // process avatar
    ...
    private function sendCommonResponse($data = [], $notify = '', $option = null)
    {
        ...
        if ($option == 'stock') {
            $response['replaceWith']['#stockTable'] = view('dashboard.partials.stock_table', $data)->render();
        }
